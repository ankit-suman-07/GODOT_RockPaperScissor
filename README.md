# RockPaperScissor



### Game Design Document: [Click Here](https://github.com/ankit-suman-07/GODOT_RockPaperScissor/blob/main/GDD.md)
### Credits: [Click Here](https://github.com/ankit-suman-07/GODOT_RockPaperScissor/blob/main/Credits.md)

## 💻 Setup locally

**Prerequisites** :
- Git 
- Godot v4.6.1

**Step 1:** Clone the repository
```
git clone https://github.com/ankit-suman-07/GODOT_RockPaperScissor.git
```

**Step 2:** Open Godot
```
In godot editor, click open a new project and browse to the cloned repository folder and select.
```

## Screenshots and Gameplay
-   **Main Menu**
![Image](/screenshots/main_menu.png)
-   **New Game**
![Image](/screenshots/start_game.png)
-   **Round Info**
![Image](/screenshots/round_info.png)
-   **Game Won**
![Image](/screenshots/winner_display.png)